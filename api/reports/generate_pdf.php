...
switch ($type) {
    ...
    $violations = $conn->query("SELECT COUNT(*) FROM violations WHERE status IN ('Open', 'In Progress')")->fetchColumn();
    ...
    $prompt = "Generate a professional executive summary for a Municipal Health & Sanitation Department. Stats: $appointments appointments, $permitsPending pending permits, $alertsActive active alerts, $violations open violations. Top diseases: $diseases. Include: 1) Overview 2) Key Findings 3) Risk Assessment 4) Recommendations. Keep under 300 words.";
    ...
    $pdf->Output('AI_Executive_Summary_' . date('Y-m-d') . '.pdf', 'D');
    exit;
    break;
...
$html .= '</table>';
$pdf->writeHTML($html, true, false, true, false, '');
$pdf->Output($type . '_report_' . date('Y-m-d_His') . '.pdf', 'D');